// src/components/DiscoveriesSection.tsx
...
const DiscoveriesSection: React.FC = () => {
  ...
  // Core: Load stored acknowldged discoveries on mount
  useEffect(() => {
    const stored = localStorage.getItem(LOCAL_STORAGE_KEY);
    if (stored) setAcknowledged(JSON.parse(stored));
  }, []);

  // Core: Toggle selection for a discovery (adds or removes id)
  ...
  // Core: Filter active discoveries that are not acknowldged
  const activeDiscoveries = allDiscoveries.filter(
    (d) => !acknowledged.some((ack) => ack.id === d.id)
  );

  // Core: Submit selected discoveries and persist them
  const handleSubmit = () => {
  ...
  // Core: Reset function to clear persisting data and reset states
  const handleReset = () => {
  ...
  // Core: Handle tab switching (active vs. acknowldged)
  ...
  // Core: Infinite scroll logic - load more items if near bottom
  useEffect(() => {
    const handleScroll = () => {
  ...
  return (
    <Container sx={{ backgroundColor: "white", p: 2 }}>
      <Stack spacing={2}>
        ...
        {/* Core: Tabs with Submit and Reset buttons grouped together */}
        <Box
          sx={{
            display: "flex",
            alignItems: "center",
